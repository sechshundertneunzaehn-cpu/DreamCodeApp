...
const Profile: React.FC<ProfileProps> = ({ userProfile, dreams, onUpdateProfile, onGenerateVideo, onGenerateNarrationVideo, onGenerateUserVoiceVideo, onPlayVideo, onUpdateDream, fontSize, language, themeMode }) => {
    ...
    const toggleCategory = (catId: string) => {
        ...
        if (catId === 'sexual') {
            ...
            if (userAge < 18) {
                alert('Diese Kategorie ist nur für Personen ab 18 Jahren verfügbar. / This category is only available for persons 18 years and older.');
                return;
            }
        }
    ...
    return (
        <div className={`max-w-2xl mx-auto pb-24 animate-in fade-in duration-500 bg-transparent min-h-screen ${textMain}`}>
                ...
                <div className="px-6 pb-6 -mt-10 relative z-10">
                    ...
                    <div className="flex justify-between items-end mb-4">
                        ...
                        <div className="flex gap-6 mb-2">
                             <div className="text-center"><span className={`block text-lg font-bold ${textMain}`}>{postCount}</span><span className="text-[10px] text-slate-400 uppercase font-bold">{t.posts}</span></div>
                             <div className="text-center"><span className={`block text-lg font-bold ${textMain}`}>{followerCount}</span><span className="text-[10px] text-slate-400 uppercase font-bold">{t.followers}</span></div>
                             <div className="text-center"><span className={`block text-lg font-bold ${textMain}`}>{followingCount}</span><span className="text-[10px] text-slate-400 uppercase font-bold">{t.following}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setIsEditing(!isEditing)} className={`flex-1 py-2.5 rounded-xl border text-xs font-bold uppercase tracking-wide ${bgElement} ${textMain}`}>{isEditing ? t.cancel : t.edit}</button>
                        <button className="flex-1 py-2.5 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white text-xs font-bold uppercase tracking-wide shadow-lg">{t.share}</button>
                    </div>
                </div>
            </div>
            ...
            <div className={`flex p-1 rounded-xl mb-6 ${bgElement}`}>
                 <button onClick={() => setActiveTab('grid')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${activeTab === 'grid' ? (isLight ? 'bg-white text-indigo-900 shadow' : 'bg-slate-800 text-white shadow') : 'text-slate-400'}`}>{t.tab_grid}</button>
                 <button onClick={() => setActiveTab('video')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${activeTab === 'video' ? (isLight ? 'bg-white text-indigo-900 shadow' : 'bg-slate-800 text-white shadow') : 'text-slate-400'}`}>{t.tab_video}</button>
                 <button onClick={() => setActiveTab('matches')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${activeTab === 'matches' ? (isLight ? 'bg-white text-indigo-900 shadow' : 'bg-slate-800 text-white shadow') : 'text-slate-400'}`}>{t.tab_matches}</button>
                 <button onClick={() => setActiveTab('audio')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${activeTab === 'audio' ? (isLight ? 'bg-white text-indigo-900 shadow' : 'bg-slate-800 text-white shadow') : 'text-slate-400'}`}>{t.tab_audio}</button>
            </div>
                ...
                <div className="space-y-3">
                    {MOCK_MATCHES.map(match => (
                        <div key={match.id} className={`p-4 rounded-2xl border flex items-center gap-4 transition-all ${isLight ? 'bg-white border-slate-200' : 'bg-slate-800/40 border-white/5'}`}>
                             ...
                             <button className="w-8 h-8 rounded-full bg-fuchsia-600 flex items-center justify-center text-white"><span className="material-icons text-sm">chat</span></button>
                        </div>
                    ))}
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={() => setShowCategoryModal(false)}>
                     <div className={`w-full max-w-sm ${isLight ? 'bg-white' : 'bg-[#1a0b2e]'} border border-fuchsia-500/30 rounded-3xl p-6 shadow-2xl`} onClick={e => e.stopPropagation()}>
                         <div className="flex justify-between items-center mb-4">
                             ...
                             <button onClick={() => setShowCategoryModal(false)} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-400"><span className="material-icons text-sm">close</span></button>
                         </div>
